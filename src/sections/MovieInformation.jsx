...
const ThemedButton = styled(Button, {
  shouldForwardProp: (prop) =>
    prop !== "isFavorite" && prop !== "isWatchlisted",
...
export default function MovieInformation() {
  ...
  const { data: recommendations } = useGetRecommendationsQuery(params.id, {
    skip: !params.id,
  });
  const { data: providers } = useGetWatchProvidersQuery(params.id, {
    skip: !params.id,
  });
  ...
  const regionProviders =
    providers?.results?.US || providers?.results?.GB || providers?.results?.CA;

  const availableProviders =
    regionProviders?.flatrate || regionProviders?.rent || regionProviders?.buy;
  ...
  const { isAuthenticated, movies = { favorites: [], watchlist: [] } } =
    useSelector(
      (state) => state.auth || { movies: { favorites: [], watchlist: [] } }
    );
            ...
            <div>
              <Stack direction="row" spacing={0.2}>
                <ThemedButton
                  ...
                  rel="noopener noreferrer"
                  size="small"
                  variant={movie.homepage ? "contained" : "disabled"}
                  endIcon={<WebsiteIcon />}
                >
                  Website
                </ThemedButton>
                ...
                {movie.videos?.results?.length > 0 && (
                  <ThemedButton
                    ...
                    rel="noopener noreferrer"
                    size="small"
                    variant={officialTrailer ? "contained" : "disabled"}
                    endIcon={<MovieIcon />}
                  >
                    Trailer
                  </ThemedButton>
